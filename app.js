...
function renderReader(m){const nm=nameFromAddress(m.from||'');const reader=$('#readerPane');const at=(m.attachments||[]).map(a=>`<span class="attachment-chip"><b>${(a.name||'FILE').split('.').pop().toUpperCase()}</b><span>${escapeHtml(a.name||'Anexo')}<small>${a.size?' · '+escapeHtml(a.size):''}</small></span><button class="attachment-download" data-id="${escapeHtml(a.id||'')}" data-name="${escapeHtml(a.name||'anexo')}" title="Baixar">${mailIcon('archive',15)}</button></span>`).join('');reader.innerHTML=`<div class="reader-head"><div class="reader-title"><h1>${escapeHtml(m.subject||'(sem assunto)')}</h1><span class="folder-pill">${escapeHtml(folderNames[m.folder]||m.folder||'')}</span></div><div class="reader-meta"><div class="sender-avatar">${initials(nm).slice(0,2)}</div><div><strong>${escapeHtml(nm)} <small style="display:inline">&lt;${escapeHtml(extractEmail(m.from||''))}&gt;</small></strong><small>para mim · ${new Date(m.date).toLocaleString('pt-BR')}</small></div><div class="reader-meta-actions"><button class="inlineStar">${m.starred?'★':'☆'}</button><button class="inlineReply">${mailIcon('reply',16)}</button><button class="inlineMore">${mailIcon('moreVertical',16)}</button></div></div></div><div class="reader-body">${normalizeBody(m.body)}</div>${at?`<div class="attachment-block"><strong>${m.attachments.length} anexo${m.attachments.length>1?'s':''}</strong><div style="margin-top:10px">${at}</div></div>`:''}`;reader.querySelector('.inlineStar').onclick=()=>toggleStar(m);reader.querySelector('.inlineReply').onclick=()=>openCompose('reply',m);reader.querySelector('.inlineMore').onclick=e=>{e.stopPropagation();showReaderContext(e,m)};reader.querySelectorAll('.attachment-download').forEach(b=>b.onclick=()=>downloadAttachment(b.dataset.id,b.dataset.name))}
