...
async function api(req,env){
  ...
  if(p==='/api/settings'&&req.method==='GET'){const s=await env.DB.prepare(`SELECT * FROM giftmail_settings WHERE user_id=?`).bind(user.id).first();return json({displayName:s?.display_name||user.display_name,email:user.email,signature:s?.signature||'',theme:s?.theme||'light',density:s?.density||'comfortable',vacation:!!s?.vacation,vacationText:s?.vacation_text||'',notifications:!!s?.notifications,sound:!!s?.sound})}
