...
async function api(req,env){
  ...
  if(p==='/api/settings'&&req.method==='GET'){const st=await env.DB.prepare(`SELECT * FROM giftmail_settings WHERE user_id=?`).bind(user.id).first();return json({displayName:st?.display_name||user.display_name,email:user.email,signatureName:st?.signature_name||st?.display_name||user.display_name,signatureCompany:st?.signature_company||'GIFT Excellence',signaturePhone:st?.signature_phone||'(31) 3773-1234',signatureCity:st?.signature_city||'Sete Lagoas - MG',signatureSite:st?.signature_site||'www.giftexcellence.com.br',signatureLogoUrl:st?.signature_logo_key?`https://giftmail-api.giftexcellence.com.br/api/public/signature-logo/${user.id}`:'https://giftmail.vercel.app/assets/gift-logo.png',signatureNew:st?.signature_new!==0,signatureReplies:!!st?.signature_replies,theme:st?.theme||'light',density:st?.density||'comfortable',vacation:!!st?.vacation,vacationText:st?.vacation_text||'',notifications:!!st?.notifications,sound:!!st?.sound})}
  if(p==='/api/settings'&&req.method==='POST'){const b=await req.json();await env.DB.prepare(`UPDATE giftmail_settings SET display_name=COALESCE(?,display_name),signature=COALESCE(?,signature),theme=COALESCE(?,theme),density=COALESCE(?,density),vacation=COALESCE(?,vacation),vacation_text=COALESCE(?,vacation_text),notifications=COALESCE(?,notifications),sound=COALESCE(?,sound),updated_at=datetime('now') WHERE user_id=?`).bind(b.displayName??null,b.signature??null,b.theme??null,b.density??null,b.vacation==null?null:(b.vacation?1:0),b.vacationText??null,b.notifications==null?null:(b.notifications?1:0),b.sound==null?null:(b.sound?1:0),user.id).run();return json({ok:true})}
